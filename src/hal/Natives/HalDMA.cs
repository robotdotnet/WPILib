...
public static partial class HalDMA
{
    ...
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static unsafe partial DMAReadStatus ReadDMADirect(void* dmaPointer, out DMASample dmaSample, double timeoutSeconds, out int remainingOut, out HalStatus status);


    [LibraryImport("wpiHal", EntryPoint = "HAL_ReadDMA")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static unsafe partial DMAReadStatus ReadDMA(HalDMAHandle handle, out DMASample dmaSample, double timeoutSeconds, out int remainingOut, out HalStatus status);
